...
void Attention<OpType>::nofused_infer(AttentionInferParam infer_param) {
  ...
  int size_per_head_half =
      (OpType == OperationType::HALF) ? size_per_head_ / 2 : size_per_head_;  // Be careful.
  ...
  if (is_remove_padding_)
    add_QKV_bias_padding<<<grid, block, 0, stream>>>(  // restore & clean zero
                                                       // for batch_gemm
        infer_param.qkv, param_.attr_bias_QKV, query, key, value, batch_size, seq_len, head_num_,
        size_per_head_ / 2, is_roformer, et_param.batch_idx, et_param.word_idx);
  else
    add_QKV_bias<<<grid, block, 0, stream>>>(infer_param.qkv, param_.attr_bias_QKV, query, key,
                                             value, batch_size, seq_len, head_num_,
                                             size_per_head_ / 2, is_roformer);
  grid.y = 1;
  // 这个时候q,k,v的数据排布变成了 head_num * B * S * size_per_head
  DataType_ alpha = (DataType_)(1.0f / sqrtf(size_per_head_ * 1.0f) / param_.tao),
            beta = (DataType_)0.0f;
  ...
  if (transformer_variety_fuse_flag_)
    variety_attention_fused_infer(
        ...
        (const __half *)infer_param.atten_mask, add_qk_buf ? (const __half *)qk_buf : NULL,
        (const __half *)infer_param.attention_bias, (__half *)infer_param.attention_output,
    ...
    if (is_remove_padding_)
      softmax_et_kernelLauncher<OpType, DataType_>(
          qk_buf, infer_param.attention_bias, infer_param.atten_mask, batch_size, seq_len,
          head_num_, stream, et_param.batch_idx, et_param.word_idx, et_param.valid_word_num);
    else
      softmax_kernelLauncher<OpType, DataType_>(qk_buf, infer_param.attention_bias,
                                                infer_param.atten_mask, batch_size, seq_len,
                                                head_num_, stream);
    ...
    if (is_remove_padding_)
      transpose_rm_padding<<<et_param.valid_word_num, block, 0, stream>>>(
          transpose_dst, infer_param.attention_output, batch_size, seq_len, head_num_,
